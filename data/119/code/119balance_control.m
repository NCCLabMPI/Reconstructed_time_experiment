...
for pitch_num = 1:2
    ...
    cats = {'face', 'object', 'letter', 'false_font'};
    cat = cats{cat_num};
        [rows, ~] = size(trial_mat(strcmp(trial_mat.pitch, pitch) & strcmp(trial_mat.category, cat),:));
        pitch_cat_mat(pitch_num,cat_num ) = rows;
    end
end
...
for pitch_num = 1:2
    ...
    cats = {'face', 'object', 'letter', 'false_font'};
    cat = cats{cat_num};
        [rows, ~] = size(trial_mat(strcmp(trial_mat.pitch, pitch) & strcmp(trial_mat.category, cat) & ~strcmp(trial_mat.trial_type, 'target'),:));
        pitch_cat_mat(pitch_num,cat_num ) = rows;
    end
end